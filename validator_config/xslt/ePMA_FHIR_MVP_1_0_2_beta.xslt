				...
				<table border="1|0">
					...
					<xsl:for-each select="$MVPelements/reference/resource[@name=$resource]/set[@name=$set]/elements/element[@min='1']">
						...
						<xsl:choose>
							...
							<xsl:when test="boolean($test_resource/*[name()=$MVPelement])">
							...
							<xsl:when test="boolean($test_resource/*[name()='contained']/*[name()=$MVPelement])">
								<xsl:variable name="result" select="boolean($test_resource//*[name()='contained']/*[name()=$MVPelement])"/>
								<xsl:call-template name="report_result">
									<xsl:with-param name="report_element" select="$MVPelement"/>
									<xsl:with-param name="report_result" select="concat($result,' (contained)')"/>
								</xsl:call-template>
							</xsl:when>						
